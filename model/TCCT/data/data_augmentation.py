...
def interaug(data, data_sl,label, batch_size, signal_length, device, num_behavioral_features,num_behavioral_features_sl):
    """
    ...
    aug_data, aug_data_sl,aug_label = [], [], []
    ...
    for cls4aug in np.unique(label):
        ...
        tmp_aug_data_sl = np.zeros((n_samples_needed, 1, num_behavioral_features_sl))
        ...
        for ri in range(n_samples_needed):
            for rj in range(n_segments):
                ...
                tmp_aug_data_sl[ri, :] = tmp_data_sl[rand_idx[rj], :]
        aug_data.append(tmp_aug_data)
        aug_data_sl.append(tmp_aug_data_sl)
        aug_label.append(np.full(n_samples_needed, cls4aug))

    # Concatenate all augmented data and labels, then shuffle
    aug_data = np.concatenate(aug_data)
    aug_data_sl = np.concatenate(aug_data_sl)
    aug_label = np.concatenate(aug_label)
    aug_shuffle = np.random.permutation(len(aug_data))
    aug_data = aug_data[aug_shuffle, :, :]
    aug_data_sl = aug_data_sl[aug_shuffle,:]
    ...
    aug_data_sl = torch.from_numpy(aug_data_sl).to(device).float()
    aug_label = torch.from_numpy(aug_label).to(device).long()
    # print(aug_data.shape)
    # print(aug_data_sl.shape)
    # print(aug_label.shape)
    return aug_data, aug_data_sl, aug_label
